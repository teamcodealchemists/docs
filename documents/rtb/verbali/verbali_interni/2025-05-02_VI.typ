...
// INSERIRE QUI CONTENUTO
  ...
  Durante la riunione è emerso un dubbio riguardante la correttezza dei casi d'uso, dato che non sono molto in linea con l'analisi dei requisiti. Alcuni requisiti sono opzionali, mentre gli analisti, per i casi d'uso che stanno considerando, ritengono che alcuni di questi siano necessari. Hanno portato l'esempio delle tipologie di utente (requisito indicato come opzionale dall'azienda proponente): considerando che arrivano degli ordini di magazzini, si può supporre di prevedere un utente di tipo cliente, oppure di gestire il tutto dall'esterno.\
  ...
  Hanno poi presentato un *API Gateway*, che si occupa di gestire le richieste e le risposte tra il Client e i vari microservizi. L'API Gateway implementa un sistema di autenticazione, che permette di accedere al sistema solo a utenti registrati, e anche un sistema di caching, che permette di memorizzare le risposte delle richieste più frequenti, in modo da velocizzare le operazioni.\

  Poi è prevista una *Gestione Centralizzata (cloud)*, che si occupa di gestire i vari microservizi essenziali per la gestione distribuita del sistema.\
  ...
  Il servizio di sincronizzazione si occupa di aggregare e mantenere sincronizzati i dati dei microservizi locali in una copia _read-only_ su cloud: serve per avere i dati a portata di mano, avendo quindi una bassa latenza. \
  ...
  Infine, ogni *magazzino (locale)* ha i microservizi in locale per far funzionare il sistema anche offline.\

  === 2.1.3 Materiale da portare all'incontro con l'azienda
  È stato deciso di portare all'incontro con l'azienda il documento prodotto dai progettisti, che contiene l'architettura del PoC.\
  ...
  Sulla base del prodotto di questa riunione sarà redatto il Diario di Bordo, che conterrà le informazioni relative alle attività svolte e ai risultati ottenuti durante la settimana.\
  ...
  Riguardo le criticità emerse e menzionate nel paragrafo 2.1.1, la discussione ha portato a una possibile soluzione: è stato pensato di inserire gli ordini manualmente, tramite l'amministratore che può individuare gli ordini. In un'ottica più ampia, si prevederebbe di avere un sistema di gestione degli ordini che arrivano dall'esterno in modo automatico.\
  Gli analisti hanno quindi deciso di avere due tipi di utente: un *amministratore globale*, che ha accesso completo al sistema, e un *supervisore locale*, che ha accesso completo a uno o più magazzini locali.\
  
  #pagebreak()
  = 3. Esiti della riunione
  La riunione si conclude con l’assegnazione dei compiti e la definizione delle decisioni e azioni intraprese, ponendo particolare attenzione al completamento dell’analisi dei requisiti#super("G") e del completamento dell'architettura#super("G"), che sarà oggetto di confronto con l’azienda nel prossimo incontro, previsto per il 5 maggio. \
  ...
  = 4. Tabella delle decisioni e azioni
    #let voci_tabella = (
    [D23], [Decisione di avere due tipi di utente: amministratore globale e supervisore locale],
    [D24], [Miglioramento del documento prodotto dai progettisti, per presentarlo all'azienda],
