...
= 1. Informazioni generali
  In data *4 aprile 2025*, dalle ore *10:00* alle ore *11:15*, si è svolta, presso la sede di _M31 Srl_ in _Via Niccolò Tommaseo, 77, 35131 Padova (PD)_ e simultaneamente in modalità virtuale tramite la piattaforma *Microsoft Teams*, una prima riunione con l'azienda _M31_ dopo l'aggiudicazione del capitolato#super[G]#super("G").
...
== 1.1 Ordine del Giorno
 ...
 + Discussione e Q&A sul capitolato#super[G]#super("G")
...
+ *Q: Cosa intendete nello specifico per _"copia remota"_ di un magazzino e perchè è necessaria? Cosa succede se un nodo#super[G] va offline? Le informazioni della copia remota di un magazzino offline possono essere utilizzate dal sistema centrale?* \
  *A:* Per copia remota si intende la visione logica di un unico magazzino distribuito, composto da più magazzini fisicamente distinti ma coordinati centralmente. Il sistema elabora i dati delle singole sedi per restituire un’unica immagine coerente della disponibilità complessiva, nascondendo la dislocazione all’utente finale. Questa impostazione è necessaria per garantire continuità e coerenza nella gestione degli ordini#super[G] anche in presenza di fault di rete o malfunzionamenti temporanei. È necessario definire le modalità di gestione dei seguenti scenari:
...
+ *Q: È necessario simulare il sistema in locale con Docker prima di utilizzare Google Cloud Platform?* \
  *A:* Risulta ottimale, se non addirittura essenziale, simulare il sistema in locale. Ci si propone l'utilizzo di *Docker*, viste anche le pregresse conoscenze del team. L'obiettivo risulta essere il funzionamento corretto del sistema in container#super[G]#super("G"). Sebbene il sistema possa essere predisposto per l'eventuale distribuzione su *Google Cloud Platform* i preferisce non configurarlo per l’uso immediato. L’uso di Minikube (simulatore di Kubernetes) è sconsigliato in quanto più complesso da configurare e non necessario per le esigenze progettuali. \
...
+ *Q: Per quanto riguarda la gestione di richieste dal sistema centralizzato: Come si intendono gestire le richieste pervenute al sistema centralizzato insieme agli ordini#super[G] che possono essere risolti localmente in modo indipendente, evitando conflitti?*\
...
+ *Q: In cosa consiste l'analisi dei sistemi di gestione dell'inventario#super[G] esistenti con focus su quelli distribuiti e basati su microservizi#super[G]? Come avviene questa fase? Saremo supervisionati? Come deve essere condotta questa analisi? Dobbiamo soffermarci su qualche sistema specifico già esistente?*\
  *A:* La prima fase del progetto, _"l’analisi allo stato dell’arte"_, richiede un approfondimento sui *sistemi di gestione dell’inventario#super[G]* esistenti, con particolare focus su quelli *distribuiti e basati su microservizi#super[G]*. Durante questa fase, il team svolgerà una ricerca interna per identificare eventuali sistemi già esistenti e studiarne l’approccio. Non è risultato utile focalizzarsi su sistemi precisi, ma è importante esplorare come altre realtà abbiano gestito la logistica distribuita e i magazzini in un contesto di microservizi#super[G].
...
+ *Q: Gli spostamenti delle merci avvengono in tempo reale? Devono essere approvati manualmente o vengono gestiti direttamente dal sistema centrale? Viene inviata una richiesta al magazzino locale che poi decide se accettarla o meno in base alle proprie disponiblità e capacità operative? Oppure il sistema centrale invia automaticamente gli ordini#super[G] di spostamento?*\
