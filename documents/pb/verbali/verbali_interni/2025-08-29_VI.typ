...
// INSERIRE QUI CONTENUTO
  ...
  Nel corso dello sprint sono stati completati i microservizi relativi alla gestione degli ordini aggregati, all’autenticazione, al sistema centralizzato e al routing.

  L’analisi e lo studio del microservizio di autenticazione mediante *ResGate* hanno evidenziato alcune criticità, principalmente riconducibili alla complessità della tecnologia e alla limitata disponibilità di documentazione ufficiale.

  In sintesi, l’architettura prevede che il client invii la richiesta di autenticazione a un microservizio del sistema, il quale, anziché rispondere direttamente al client _(operazione che potrebbe comportare potenziali rischi di sicurezza)_ inoltra la richiesta a ResGate. Quest’ultimo, tramite l’utilizzo di un token, verifica la validità della connessione e delle credenziali dell’utente. Successivamente, ogni richiesta verso i microservizi del sistema viene accompagnata dal token generato da ResGate, che consente al microservizio destinatario di validare i permessi e autorizzare l’accesso alle risorse richieste.

  === Codifica dei microservizi
  Nel corso dello sprint è stato completato il microservizio di inventario, sia per quanto concerne l’implementazione che le attività di testing. Parallelamente, lo sviluppo del microservizio di ordini risulta attualmente in fase di completamento.
  ...
  - Completamento del microservizio Orders e relativa fase di test unitario.
  - Implementazione del microservizio Ordine Aggregato e test unitario.
  - Completamento della Specifica Tecnica.
  - Implementazione del microservizio Stato.
  - Implementazione del microservizio Stato Cloud.
  - Integrazione del microservizio di Autenticazione con gli altri microservizi, comprensiva di test unitari e test di accettazione.
  - Implementazione del microservizio Sistema Centralizzato.
  - Studio dei sistemi di telemetria e logging e loro integrazione nei microservizi.

  Per ciascun membro, è raccomandato di consultare i requisiti relativi al proprio microservizio, aggiornare la *Checklist Requisiti Soddisfatti* spuntando quelli completati e pianificare i relativi test di accettazione e/o casi d’uso.

  Nella seconda parte dello sprint, saranno svolte le attività di verifica e completamento della documentazione esistente e di compilazione del *Manuale Utente*.
...
#pagebreak()
  = Tabella delle decisioni e azioni
    ...
    [D45], [Aumentare il numero di programmatori per terminare la fase di codifica.],
