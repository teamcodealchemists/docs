...
#let voci_test_unità = (
    ...
    [TdU75], [emitAccessToken chiama la porta con token e cid], [token e cid passati correttamente], [TS],
...
#let voci_test_unità = (
    ...
    [TdU112], [Dovrebbe chiamare handleNotification], [handleNotification chiamato con messaggio], [TS],
...
#let voci_test_unità = (
    [TdU220], [publishHeartbeat chiama outboundService.publishHeartbeat con DTO], [publishHeartbeat invocato correttamente], [TS],
    [TdU221], [publishState chiama outboundService.publishState con DTO], [publishState invocato correttamente], [TS],
    [TdU222], [stateUpdated chiama outboundService.stateUpdated con DTO], [stateUpdated invocato correttamente], [TS],
    [TdU223], [getState restituisce CloudWarehouseState se trovato], [ritorna stato ONLINE], [TS],
    [TdU224], [getState restituisce null se non trovato], [ritorna null], [TS],
    [TdU225], [updateState restituisce true se acknowledged], [ritorna true], [TS],
    [TdU226], [updateState restituisce false se non acknowledged], [ritorna false], [TS],
    [TdU227], [getAllWarehouseIds restituisce array di CloudWarehouseId], [ritorna array di ID], [TS],
    [TdU228], [getState chiama il repository], [repository invocato e ritorna ONLINE], [TS],
    [TdU229], [updateState chiama il repository], [repository invocato e ritorna true], [TS],
    [TdU230], [handleHeartbeatResponse chiama tutte le callback e le pulisce], [callback invocate e lista svuotata], [TS],
    [TdU231], [checkHeartbeat imposta lo stato ONLINE se isAlive è true], [ritorna ONLINE], [TS],
    [TdU232], [checkHeartbeat imposta lo stato OFFLINE se non arriva risposta], [ritorna OFFLINE], [TS],
    [TdU233], [startPeriodicHeartbeatCheck chiama checkHeartbeat per ogni magazzino], [checkHeartbeat chiamato per tutti gli ID], [TS],
    [TdU234], [chiama handleHeartbeatResponse con i parametri corretti], [ritorna OK], [TS],
    [TdU235], [gestisce errori e restituisce un JSON di errore], [ritorna errore system.invalidParams], [TS],
    [TdU236], [updateState aggiorna lo stato se diverso e chiama notifyStateUpdated], [ritorna Address updated successfully], [TS],
    [TdU237], [updateState non aggiorna lo stato se uguale], [ritorna Address updated successfully], [TS],
    [TdU238], [updateState gestisce errori e restituisce un JSON di errore], [ritorna errore system.invalidParams], [TS],
    [TdU239], [getState restituisce lo stato e chiama publishState], [ritorna stato ONLINE], [TS],
    [TdU240], [getState restituisce stringa se non trova lo stato], [ritorna No state found], [TS],
    [TdU241], [getState gestisce errori e restituisce un JSON di errore], [ritorna errore system.invalidParams], [TS],
    [TdU242], [chiama logger.verbose con il messaggio corretto], [logger.verbose invocato con messaggio e dati], [TS],
    [TdU243], [restituisce un oggetto con pattern, data e id], [ritorna oggetto con data e id stringa], [TS],
    [TdU244], [cloudWarehouseIdToDomain converte DTO in Domain], [ritorna CloudWarehouseId], [TS],
    [TdU245], [cloudWarehouseStateToDomain converte DTO in Domain], [ritorna CloudWarehouseState ONLINE], [TS],
    [TdU246], [cloudHeartbeatToDomain converte DTO in Domain], [ritorna CloudHeartbeat ALIVE], [TS],
    [TdU247], [cloudWarehouseIdToDTO converte Domain in DTO], [ritorna DTO con warehouseId], [TS],
    [TdU248], [cloudWarehouseStateToDTO converte Domain in DTO], [ritorna DTO con warehouseId e state], [TS],
    [TdU249], [cloudHeartbeatToDTO converte Domain in DTO], [ritorna DTO con heartbeatMsg e timestamp], [TS],
    [TdU250], [publishHeartbeat chiama natsService.publish con il topic corretto], [natsService.publish invocato con heartbeat], [TS],
    [TdU251], [publishState chiama natsService.publish con il topic corretto], [natsService.publish invocato con state JSON], [TS],
    [TdU252], [stateUpdated chiama natsService.publish con il topic corretto], [natsService.publish invocato con state], [TS],
    [TdU253], [CloudWarehouseStateDTO valida un DTO corretto], [nessun errore], [TS],
    [TdU254], [CloudWarehouseStateDTO non valida se warehouseId è negativo], [errore su warehouseId], [TS],
    [TdU255], [CloudWarehouseStateDTO non valida se warehouseId è mancante], [errore su warehouseId], [TS],
    [TdU256], [CloudWarehouseStateDTO non valida se state è mancante], [errore su state], [TS],
    [TdU257], [CloudWarehouseStateDTO non valida se state non è una stringa], [errore su state], [TS],
    [TdU258], [crea la microservice app con le opzioni corrette], [microservice creato con NATS e server configurato], [TS],
    [TdU259], [deve avere la struttura corretta], [config NATS contiene tutte le proprietà richieste], [TS],
    [TdU260], [deve usare NATS_URL se presente], [config NATS usa URL custom], [TS],
    [TdU261], [deve usare il valore di default se NATS_URL non è presente], [config NATS usa URL di default], [TS],
    [TdU262], [onModuleInit inizializza la connessione NATS], [connessione NATS inizializzata], [TS],
    [TdU263], [onModuleDestroy chiude la connessione NATS], [connessione NATS chiusa], [TS],
    [TdU264], [publish invia il messaggio codificato], [messaggio codificato e pubblicato], [TS],
    [TdU265], [serializza correttamente un oggetto con proprietà data], [ritorna oggetto con proprietà data], [TS],
    [TdU266], [serializza un oggetto senza proprietà data], [ritorna oggetto con data undefined], [TS],
    [TdU267], [logga la serializzazione], [logger.verbose chiamato con messaggi di serializzazione], [TS],
    [TdU268], [serializza una risposta con response], [ritorna data con response e loggata], [TS],
    [TdU269], [serializza una risposta con err come stringa], [ritorna error JSON e loggata], [TS],
    [TdU270], [serializza una risposta con err come array di errori di validazione], [ritorna error JSON concatenato e loggata], [TS],
    [TdU271], [serializza una risposta senza response né err], [ritorna data originale e loggata], [TS]
  )
#let caption_test_unità = (
...
#let voci_test_unità = (
  [TdU272], [deve chiamare outboundService.sendWarehouseAndState con il DTO corretto], [chiamata a outboundService], [TS],
...
#let voci_test_unità = (
  [TdU339], [gestisce errori e restituisce JSON formattato], [estrae messaggi da array di errori o usa il messaggio di un errore generico e ritorna JSON con code e meta appropriati], [TS],
...
#let voci_test_unità = (
    [TdU400], [stockReserved chiama ordersService con id e items], [syncUpdateReservedStock invocato con dto.id e dto.items], [TS],
    [TdU401], [syncAddSellOrder chiama ordersService con ordine], [syncCreateSellOrder invocato con dto], [TS],
    [TdU402], [syncAddInternalOrder chiama ordersService con ordine interno], [syncCreateInternalOrder invocato con dto], [TS],
    [TdU403], [updateOrderState aggiorna lo stato dell'ordine], [syncUpdateOrderState invocato con id e stato], [TS],
    [TdU404], [cancelOrder cancella un ordine], [syncCancelOrder invocato con id], [TS],
    [TdU405], [completeOrder completa un ordine], [syncUpdateOrderState invocato con id e COMPLETED], [TS],
    [TdU406], [getOrderState recupera stato ordine dal repository e converte DTO], [repository e dataMapper restituiscono DTO con stato], [TS],
    [TdU407], [getOrder recupera un ordine interno], [repository e dataMapper restituiscono InternalOrderDTO], [TS],
    [TdU408], [getOrder recupera un ordine di vendita], [repository e dataMapper restituiscono SellOrderDTO], [TS],
    [TdU409], [getOrder lancia errore se tipo non riconosciuto], [errore lanciato], [TS],
    [TdU410], [getAllFilteredOrders recupera ordini filtrati], [repository e dataMapper restituiscono DTO array], [TS],
    [TdU411], [getAllOrders recupera tutti gli ordini], [repository e dataMapper restituiscono DTO array], [TS],
    [TdU412], [orderUpdated pubblica aggiornamento ordine su NATS], [natsService.publish chiamato con topic e ordine], [TS],
    [TdU413], [publishAllProducts pubblica tutti i prodotti], [natsService.publish chiamato con topic e oggetti orders], [TS],
    [TdU414], [orderCancelled pubblica cancellazione ordine], [natsService.publish chiamato con topics e orderId], [TS],
    [TdU415], [orderCompleted pubblica completamento ordine], [natsService.publish chiamato con topics e orderId], [TS],
    [TdU416], [grant access per token globale], [accesso consentito], [TS],
    [TdU417], [grant access per warehouse assegnato], [accesso consentito], [TS],
    [TdU418], [deny access per warehouse non assegnato], [accesso negato], [TS],
    [TdU419], [deny access se token ha errore], [accesso negato con codice e messaggio di errore], [TS],
    [TdU420], [deny access se token mancante], [accesso negato con codice e messaggio di errore], [TS],
    [TdU421], [chiama stockReserved con payload], [inboundPortsAdapter.stockReserved invocato con payload], [TS],
    [TdU422], [chiama syncAddSellOrder con payload], [inboundPortsAdapter.syncAddSellOrder invocato con payload], [TS],
    [TdU423], [chiama syncAddInternalOrder con payload], [inboundPortsAdapter.syncAddInternalOrder invocato con payload], [TS],
    [TdU424], [chiama updateOrderState con payload], [inboundPortsAdapter.updateOrderState invocato con id e stato], [TS],
    [TdU425], [chiama cancelOrder con payload], [inboundPortsAdapter.cancelOrder invocato con id], [TS],
    [TdU426], [chiama completeOrder con payload], [inboundPortsAdapter.completeOrder invocato con id], [TS],
    [TdU427], [chiama getOrderState e ritorna valore], [inboundPortsAdapter.getOrderState restituisce DTO con stato], [TS],
    [TdU428], [chiama getOrder per InternalOrder], [repository e dataMapper restituiscono InternalOrderDTO], [TS],
    [TdU429], [chiama getOrder per SellOrder], [repository e dataMapper restituiscono SellOrderDTO], [TS],
    [TdU430], [getOrder lancia errore se tipo non riconosciuto], [errore lanciato], [TS],
    [TdU431], [getAllFilteredOrders chiama repository e dataMapper], [restituisce collection con rid], [TS],
    [TdU432], [getAllOrders chiama repository e dataMapper], [restituisce collection con rid], [TS],
    [TdU433], [getAllOrdersForCentralized chiama repository e restituisce collection], [restituisce collection con orderId], [TS],
    [TdU434], [getAllOrdersForCentralized restituisce null se repository vuota], [result null], [TS],
    [TdU435], [SyncOrderIdDTO valido], [nessun errore], [TS],
    [TdU436], [SyncOrderIdDTO non valido], [errore di validazione], [TS],
    [TdU437], [SyncItemIdDTO valido], [nessun errore], [TS],
    [TdU438], [SyncItemIdDTO non valido], [errore di validazione], [TS],
    [TdU439], [SyncOrderItemDTO valido], [nessun errore], [TS],
    [TdU440], [SyncOrderItemDTO non valido], [errore di validazione], [TS],
    [TdU441], [SyncOrderItemDetailDTO valido], [nessun errore], [TS],
    [TdU442], [SyncOrderItemDetailDTO non valido], [errore di validazione], [TS],
    [TdU443], [SyncOrderStateDTO valido], [nessun errore], [TS],
    [TdU444], [SyncOrderStateDTO non valido], [errore di validazione], [TS],
    [TdU445], [SyncSellOrderDTO valido], [nessun errore], [TS],
    [TdU446], [SyncSellOrderDTO non valido], [errore di validazione], [TS],
    [TdU447], [SyncInternalOrderDTO valido], [nessun errore], [TS],
    [TdU448], [SyncInternalOrderDTO non valido], [errore di validazione], [TS],
    [TdU449], [SyncOrderQuantityDTO valido], [nessun errore], [TS],
    [TdU450], [SyncOrderQuantityDTO non valido], [errore di validazione], [TS],
    [TdU451], [SyncOrdersDTO valido], [nessun errore], [TS],
    [TdU452], [SyncOrdersDTO non valido], [errore di validazione], [TS],
    [TdU453], [Logger verbose log del deserializer], [log corretto chiamato], [TS],
    [TdU454], [Deserializer restituisce oggetto con pattern, data e id], [oggetto con pattern/data/id], [TS],
    [TdU455], [Bootstrap crea microservice con opzioni corrette], [app creata con NATS e server], [TS],
    [TdU456], [syncOrderIdToDomain converte DTO in dominio], [istanza SyncOrderId con id], [TS],
    [TdU457], [syncOrderStateToDomain accetta solo stati validi], [stato valido o errore], [TS],
    [TdU458], [syncOrderItemToDomain converte DTO in dominio], [istanza SyncOrderItem], [TS],
    [TdU459], [syncOrderItemDetailToDomain valida quantityReserved], [istanza o errore se invalido], [TS],
    [TdU460], [syncInternalOrderToDomain valida magazzini diversi], [istanza o errore se magazzini uguali], [TS],
    [TdU461], [syncSellOrderToDomain converte correttamente], [istanza SyncSellOrder], [TS],
    [TdU462], [syncOrderIdToDTO converte dominio in DTO], [oggetto con id], [TS],
    [TdU463], [syncOrderStateToDTO converte dominio in DTO], [oggetto con orderState], [TS],
    [TdU464], [syncOrderItemToDTO converte dominio in DTO], [oggetto itemId/quantity], [TS],
    [TdU465], [syncOrderItemDetailToDTO converte dominio in DTO], [oggetto item/quantity/unitPrice], [TS],
    [TdU466], [syncInternalOrderToDTO converte dominio in DTO], [oggetto con id e magazzini], [TS],
    [TdU467], [syncSellOrderToDTO converte dominio in DTO], [oggetto con id e magazzino/destinazione], [TS],
    [TdU468], [syncOrdersToDTO converte collezione di ordini], [array sell/internal con id], [TS],
    [TdU469], [NATS config struttura corretta], [tutte le proprietà presenti], [TS],
    [TdU470], [NATS usa NATS_URL se presente], [server custom], [TS],
    [TdU471], [NATS usa default se NATS_URL non presente], [server localhost], [TS],
    [TdU472], [NatsService onModuleInit inizializza connessione], [nc definito], [TS],
    [TdU473], [NatsService onModuleDestroy chiude connessione], [close chiamato], [TS],
    [TdU474], [NatsService publish invia messaggio codificato], [messaggio codificato pubblicato], [TS],
    [TdU475], [syncUpdateOrderState chiama repository con parametri corretti], [chiamata repository con id e stato], [TS],
    [TdU476], [syncCreateSellOrder chiama repository con nuovo ordine], [repository chiamata con ordine con id corretto], [TS],
    [TdU477], [syncCreateInternalOrder chiama repository con nuovo ordine], [repository chiamata con ordine con id corretto], [TS],
    [TdU478], [syncCancelOrder chiama repository e logga corretto], [repository chiamata con id], [TS],
    [TdU479], [syncCancelOrder logga errore se repository restituisce false], [repository chiamata con id], [TS],
    [TdU480], [syncUpdateReservedStock chiama repository con parametri corretti], [repository chiamata con id e items], [TS],
    [TdU481], [SyncOrderId restituisce id], [ritorna id corretto], [TS],
    [TdU482], [SyncOrderId restituisce tipo ordine], [ritorna prima lettera dell'id], [TS],
    [TdU483], [SyncItemId restituisce id], [ritorna id corretto], [TS],
    [TdU484], [SyncOrderItem restituisce quantity], [ritorna quantity corretta], [TS],
    [TdU485], [Serializer serializza risposta con response], [data correttamente formattata], [TS],
    [TdU486], [Serializer serializza risposta con err come stringa], [data con errore formattato], [TS],
    [TdU487], [Serializer serializza risposta con err come array], [data con errori concatenati], [TS],
    [TdU488], [Serializer serializza risposta senza response né err], [data invariata], [TS],
    [TdU489], [getById restituisce InternalOrder se trovato], [ritorna InternalOrder con id corretto], [TS],
    [TdU490], [getById restituisce SellOrder se InternalOrder non trovato], [ritorna SellOrder con id corretto], [TS],
    [TdU491], [getById lancia errore se nessun ordine trovato], [errore "Ordine con ID ... non trovato"], [TS],
    [TdU492], [getState restituisce stato di InternalOrder se trovato], [ritorna stato corretto], [TS],
    [TdU493], [getState restituisce stato di SellOrder se InternalOrder non trovato], [ritorna stato corretto], [TS],
    [TdU494], [getState lancia errore se nessun ordine trovato], [errore "Stato per l'ordine ... non trovato"], [TS],
    [TdU495], [getAllFilteredOrders restituisce SyncOrders filtrati], [ritorna collezione interna e sell con lunghezza corretta], [TS],
    [TdU496], [syncRemoveById ritorna false se ordine già cancellato], [ritorna false], [TS],
    [TdU497], [syncRemoveById ritorna true se cancellazione riuscita], [ritorna true], [TS],
    [TdU498], [syncUpdateOrderState aggiorna InternalOrder], [ritorna InternalOrder aggiornato], [TS],
    [TdU499], [syncUpdateOrderState aggiorna SellOrder se InternalOrder non trovato], [ritorna SellOrder aggiornato], [TS],
    [TdU500], [syncUpdateOrderState lancia errore se nessun ordine trovato], [errore], [TS],
    [TdU501], [syncUpdateReservedStock lancia errore se nessun ordine trovato], [errore], [TS],
    [TdU502], [syncUnreservedStock aggiorna tutti gli items di InternalOrder], [bulkWrite chiamato, stock resettato], [TS],
    [TdU503], [syncUnreservedStock aggiorna tutti gli items di SellOrder], [bulkWrite chiamato, stock resettato], [TS],
    [TdU504], [syncUnreservedStock lancia errore se nessun documento trovato dopo bulkWrite], [errore "Impossibile trovare l'ordine ..."], [TS],
    [TdU505], [syncUpdateReservedStock lancia errore se item non trovato], [errore "Impossibile trovare l'ordine ..."], [TS],
  )
#let caption_test_unità = (
...
[TdU714], [Verifica esistenza ordine], [Restituisce true se l'ordine esiste], [TS],
[TdU715], [Aggiorna stock per SellOrder], [updateReservedStock e publishUpdatedReservedStock vengono chiamati], [TS],
[TdU716], [Aggiorna stock per InternalOrder], [updateReservedStock e publishUpdatedReservedStock vengono chiamati], [TS],
[TdU717], [Errore durante updateReservedStock], [Viene sollevata un'eccezione 'DB error'], [TS],
[TdU718], [Aggiorna stock completo per SellOrder], [updateReservedStock viene chiamato], [TS],
[TdU719], [Aggiorna stock completo per InternalOrder], [updateReservedStock viene chiamato], [TS],
[TdU720], [Aggiorna stato ordine con successo], [updateOrderState e orderStateUpdated vengono chiamati], [TS],
[TdU721], [Transizione stato non valida], [Viene sollevata un'eccezione per flusso non corretto], [TS],
[TdU722], [Aggiorna stato da magazzino non di partenza], [orderStateUpdated NON viene chiamato], [TS],
[TdU723], [Crea InternalOrder in magazzino destinazione], [Restituisce ID dell'ordine e waitingForStock viene chiamato], [TS],
[TdU724], [Annulla ordine InternalOrder], [updateOrderState e unreserveStock vengono chiamati], [TS],
[TdU725], [Stock riservato per SellOrder], [updateReservedStock viene chiamato], [TS],
[TdU726], [Controllo riserva completa SellOrder], [publishShipment viene chiamato], [TS],
[TdU727], [Controllo riserva incompleta SellOrder], [publishStockRepl viene chiamato], [TS],
[TdU728], [Controllo riserva completa InternalOrder], [publishInternalOrder viene chiamato], [TS],
[TdU729], [Controllo riserva incompleta InternalOrder], [cancelOrder viene chiamato], [TS],
[TdU730], [Spedizione SellOrder], [updateOrderState(SHIPPED) e publishShipment vengono chiamati], [TS],
[TdU731], [Spedizione InternalOrder], [orderStateUpdated viene chiamato], [TS],
[TdU732], [Ricezione InternalOrder], [receiveShipment viene chiamato], [TS],
[TdU733], [Ricezione SellOrder], [receiveShipment NON viene chiamato], [TS],
[TdU734], [Completamento SellOrder], [updateOrderState(COMPLETED) viene chiamato], [TS],
    [TdU735], [Completamento InternalOrder nel magazzino di destinazione], [orderCompleted e publishReserveStock vengono chiamati], [TS],
    [TdU736], [Completamento InternalOrder in altro magazzino], [orderCompleted NON viene chiamato], [TS],
    [TdU737], [updateReservedStock con items vuoto], [updateReservedStock viene comunque chiamato], [TS],
    [TdU738], [updateReservedStock con item non corrispondenti], [updateReservedStock viene chiamato comunque], [TS],
    [TdU739], [Gestione evento stockReserved], [stockReserved viene chiamato con il DTO], [TS],
    [TdU740], [Aggiunta SellOrder], [Restituisce ID e viene chiamato addSellOrder], [TS],
    [TdU741], [Errore durante aggiunta SellOrder], [Restituisce messaggio di errore con 'system.internalError'], [TS],
    [TdU742], [Aggiunta InternalOrder], [Restituisce ID e viene chiamato addInternalOrder], [TS],
    [TdU743], [Errore durante aggiunta InternalOrder], [Restituisce messaggio di errore con 'system.internalError'], [TS],
    [TdU744], [Gestione evento addInternalOrderEvent], [Restituisce ID ordine correttamente], [TS],
    [TdU745], [Disponibilità sufficiente prodotti], [Viene chiamato sufficientProductAvailability], [TS],
    [TdU746], [Gestione evento waitingForStock], [Viene chiamato waitingForStock con ID ordine], [TS],
    [TdU747], [Gestione evento stockShipped], [Viene chiamato stockShipped con ID ordine], [TS],
    [TdU748], [Gestione evento stockReceived], [Viene chiamato stockReceived], [TS],
    [TdU749], [Gestione evento replenishmentReceived], [Viene chiamato replenishmentReceived], [TS],
    [TdU750], [Aggiornamento stato ordine riuscito], [Restituisce messaggio di stato aggiornato], [TS],
    [TdU751], [Errore aggiornamento stato ordine], [Restituisce 'system.internalError'], [TS],
    [TdU752], [Aggiornamento stato da evento], [Viene chiamato updateOrderState con ID e stato], [TS],
    [TdU753], [Errore aggiornamento stato da evento], [Sollevata eccezione con messaggio 'Update error'], [TS],
    [TdU754], [Annullamento ordine riuscito], [Viene chiamato cancelOrder con ID], [TS],
    [TdU755], [Errore durante annullamento ordine], [Sollevata RpcException], [TS],
    [TdU756], [Completamento ordine], [Viene chiamato completeOrder con ID], [TS],
    [TdU757], [Recupero stato ordine], [Restituisce stato 'PENDING' e chiama getOrderState], [TS],
    [TdU758], [Recupero ordine SellOrder], [Restituisce S123 e 'Via Roma'], [TS],
    [TdU759], [Recupero ordine InternalOrder], [Restituisce I123 e contiene 'warehouseDestination'], [TS],
    [TdU760], [Recupero lista ordini], [Restituisce entrambi SellOrder e InternalOrder], [TS],
    [TdU761], [InternalOrder con sellOrderReference vuoto], [Ordine aggiunto con successo], [TS],
    [TdU762], [Parsing contesto con warehouse diverso], [updateOrderState chiamato con ID e stato corretti], [TS],
    [TdU763], [Connessione a NATS riuscita], [connect viene chiamato senza errori], [TS],
    [TdU764], [Errore durante connessione NATS], [Log dell'errore con messaggio 'Error connecting to NATS service'], [TS],
    [TdU765], [Evento waitingForStock pubblicato], [emit chiamato con subject corretto], [TS],
    [TdU766], [Pubblicazione evento reserveStock], [emit chiamato con subject e payload corretti], [TS],
    [TdU767], [Pubblicazione evento updatedReservedStock], [emit chiamato con subject e payload corretti], [TS],
    [TdU768], [Pubblicazione evento unreserveStock], [emit chiamato due volte per inventario e aggregate], [TS],
    [TdU769], [Pubblicazione evento shipment], [emit chiamato con subject e payload corretti], [TS],
    [TdU770], [Pubblicazione evento stock replenishment], [emit chiamato con subject e payload corretti], [TS],
    [TdU771], [Pubblicazione evento receiveShipment], [emit chiamato con subject corretto basato su warehouse], [TS],
    [TdU772], [Aggiornamento stato ordine verso aggregate], [emit chiamato con subject aggregate e payload vuoto], [TS],
    [TdU773], [Aggiornamento stato ordine verso warehouse], [emit chiamato con subject warehouse e payload vuoto], [TS],
    [TdU774], [Errore durante aggiornamento stato ordine], [Viene sollevato errore con messaggio 'NATS error'], [TS],
    [TdU775], [Pubblicazione evento ordine annullato], [emit chiamato con subject cancel], [TS],
    [TdU776], [Errore pubblicazione ordine annullato], [Viene sollevato errore 'NATS error'], [TS],
    [TdU777], [Pubblicazione ordine completato], [emit chiamato con subject complete], [TS],
    [TdU778], [Errore pubblicazione ordine completato], [Viene sollevato errore 'Mapping error'], [TS],
    [TdU779], [Pubblicazione InternalOrder verso aggregate], [emit chiamato con subject aggregate e payload DTO], [TS],
    [TdU780], [Pubblicazione InternalOrder verso warehouse], [emit chiamato con subject warehouse e payload DTO], [TS],
    [TdU781], [Pubblicazione SellOrder verso aggregate], [emit chiamato con subject aggregate e contiene ID ordine], [TS],
    [TdU782], [SellOrder restituisce ID come stringa], [Risultato è '"S123"'], [TS],
    [TdU783], [Gestione items array vuoto in publishReserveStock], [emit chiamato con itemsDTO vuoto], [TS],
    [TdU784], [Gestione items multipli in publishReserveStock], [emit chiamato con due itemsDTO], [TS],
    [TdU785], [Gestione warehouseId diverso da 1], [emit chiamato con subject 'warehouse.3.order.request'], [TS],
    [TdU786], [Serializzazione richiesta con proprietà 'data' oggetto], [Risultato uguale all'oggetto iniziale], [TS],
    [TdU787], [Serializzazione richiesta con 'data' vuoto], [Risultato è { data: {} }], [TS],
    [TdU788], [Serializzazione richiesta con 'data' come stringa], [Risultato è { data: 'test' }], [TS],
    [TdU789], [Serializzazione con proprietà 'response'], [Ritorna { data: { foo: 'bar' } } + logging verbose], [TS],
    [TdU790], [Serializzazione con errore come stringa], [Ritorna JSON con errore (code: system.invalidParams, message: errore generico)], [TS],
    [TdU791], [Serializzazione con errore come array di errori di validazione], [Ritorna JSON con errore aggregato dai constraint + logging], [TS],
    [TdU792], [Serializzazione fallback (nessun response né err)], [Ritorna { data: 'test' } + logging verbose], [TS],
    [TdU793], [startSellOrderSaga chiama publishReserveStock con items corretti], [Chiama repository, estrae item e chiama publishReserveStock], [TS],
    [TdU794], [startInternalOrderSaga chiama publishReserveStock con items corretti], [Chiama repository, estrae item e chiama publishReserveStock], [TS],
    [TdU795], [Gestione errore da getById], [startSellOrderSaga e startInternalOrderSaga lanciano errore se getById fallisce], [TS],
    [TdU796], [Gestione errore da publishReserveStock], [startSellOrderSaga e startInternalOrderSaga lanciano errore se publishReserveStock fallisce], [TS]
)
...
#let voci_test_unità = (
  [TdU797], ["Crea un DTO valido da oggetto plain"], ["DTO valido, nessun errore di validazione"], [TS],
  [TdU798], ["Fallisce la validazione se manca lo stato"], ["errors[0].property = 'state'"], [TS],
  [TdU799], ["Fallisce la validazione se lo stato non è una stringa"], ["errors[0].property = 'state'"], [TS],
  [TdU800], ["Fallisce la validazione se manca warehouseId"], ["errors[0].property = 'warehouseId'"], [TS],
  [TdU801], ["Service definito"], ["service definito"], [TS],
  [TdU802], ["Crea Heartbeat con messaggio corretto usando syncHeartbeat"], ["heartbeat contiene 'ONLINE', id corretto, timestamp è Date"], [TS],
  [TdU803], ["Chiama publishHeartbeat con Heartbeat usando sendHeartBeat"], ["publishHeartbeat chiamato con Heartbeat, dati corretti"], [TS],
  [TdU804], ["Crea documento State con tutte le proprietà"], ["doc ha proprietà warehouseId, state, lastHeartbeat, lastHeartbeatMsg"], [TS],
  [TdU805], ["Restituisce WarehouseState dal documento"], ["result è WarehouseState, stato = 'ACTIVE'"], [TS],
  [TdU806], ["Restituisce stato 'unknown' se non trovato"], ["result è WarehouseState, stato = 'unknown'"], [TS],
  [TdU807], ["Restituisce true se update acknowledged"], ["result = true"], [TS],
  [TdU808], ["Restituisce false se update non acknowledged"], ["result = false"], [TS],
  [TdU809], ["Chiama getSyncedState con input oggetto"], ["getSyncedState chiamato con { id: 5 }"], [TS],
  [TdU810], ["Chiama getSyncedState con input stringa"], ["getSyncedState chiamato con { id: 10 }"], [TS],
  [TdU811], ["Default a warehouseId 0 se input malformato"], ["getSyncedState chiamato con { id: 0 }"], [TS],
  [TdU812], ["Gestisce stringa JSON non valida"], ["logger.error chiamato, getSyncedState con { id: 0 }"], [TS],
  [TdU813], ["Chiama connect su onModuleInit"], ["connect chiamato"], [TS],
  [TdU814], ["Pubblica heartbeat"], ["emit chiamato con topic heartbeat e dto"], [TS],
  [TdU815], ["Pubblica stato magazzino"], ["emit chiamato con topic stato e dati"], [TS],
  [TdU816], ["Pubblica stato aggiornato"], ["emit chiamato con topic updated e dati"], [TS],
  [TdU817], ["Pubblica stato magazzino tramite adapter"], ["publishState chiamato con warehouseId e state"], [TS],
  [TdU818], ["Pubblica heartbeat tramite adapter"], ["publishHeartbeat chiamato con HeartbeatDTO"], [TS],
  [TdU819], ["Logga errore se publishState fallisce"], ["logger.error chiamato"], [TS],
  [TdU820], ["Logga errore se publishHeartbeat fallisce"], ["logger.error chiamato"], [TS],
  [TdU821], ["Chiama sendHeartBeat con oggetti dominio corretti"], ["sendHeartBeat chiamato con WarehouseId e WarehouseState 'ONLINE'"], [TS],
  [TdU822], ["Crea entità Heartbeat"], ["getHeartbeatMsg = 'ALIVE', getId = 1"], [TS],
  [TdU823], ["Crea entità WarehouseState"], ["getState = 'ACTIVE', dopo setState = 'INACTIVE'"], [TS],
  [TdU824], ["Crea entità WarehouseId"], ["getId = 42"], [TS],
  [TdU825], ["Mappa WarehouseIdDTO a WarehouseId"], ["entity è WarehouseId, id = 5"], [TS],
  [TdU826], ["Mappa WarehouseStateDTO a WarehouseState"], ["entity è WarehouseState, stato = 'ACTIVE'"], [TS],
  [TdU827], ["Mappa HeartbeatDTO a Heartbeat"], ["entity è Heartbeat, msg = 'ALIVE', timestamp corretto, id = 1"], [TS],
  [TdU828], ["Mappa WarehouseId a WarehouseIdDTO"], ["dto = { id: 10 }"], [TS],
  [TdU829], ["Mappa WarehouseState a WarehouseStateDTO"], ["dto.state = 'ACTIVE', dto.warehouseId.id = 3"], [TS],
  [TdU830], ["Mappa Heartbeat a HeartbeatDTO"], ["dto è HeartbeatDTO, msg = 'ALIVE', warehouseId = 7, timestamp è Date"], [TS]
  )
#let caption_test_unità = (
